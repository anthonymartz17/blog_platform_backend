...
func (h *Handler) RegisterRoutes(r *mux.Router, authVerifier middleware.AuthVerifier) {

	r.HandleFunc("/posts", h.GetPosts).Methods(http.MethodGet)

	protected := r.PathPrefix("/").Subrouter()
	protected.Use(middleware.AuthMiddleware(authVerifier))
...
func (h *Handler) GetPosts(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		httpResponse.Error(w, http.StatusInternalServerError, errMsg)
		return
	}

	httpResponse.JSON(w, http.StatusOK, posts)
...
func (h *Handler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if err := validatePayload(&payload); err != nil {
		...
		httpResponse.Error(w, http.StatusBadRequest, msgEmptyContent)
		return
	}
	...
	if !ok {
		...
		httpResponse.Error(w, http.StatusUnauthorized, msgUnauthorized)
		return
	}
	...
	if err != nil {
		...
		if errors.Is(err, context.DeadlineExceeded) {
			...
			httpResponse.Error(w, http.StatusGatewayTimeout, msgInternal)
			return
		}
	...
	httpResponse.JSON(w, http.StatusCreated, newPost)
}
